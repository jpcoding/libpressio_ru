...
struct impl_compress {
  template <class T>
  typename std::enable_if<!std::is_same<T, bool>::value, pressio_data>::type operator()(T *in_data,
                                                                                        T *) {
    config.N = static_cast<char>(reg_dims.size());
    config.num =
        std::accumulate(reg_dims.begin(), reg_dims.end(), (size_t)1, compat::multiplies<>());
    ...
    return pressio_data::move(pressio_byte_dtype, compressedData, {outSize},
                              pressio_new_free_fn<QoZ::uchar>(), nullptr);
  }
  pressio_data operator()(bool *, bool *) { throw std::runtime_error("unsupported type bool"); }
...
template <class K, class V, class Sort> std::vector<K> keys(std::map<K, V, Sort> const &map) {
  std::vector<K> k;
  k.reserve(map.size());
...
public:
  ...
  struct pressio_options get_options_impl() const override {
    ...
    // set_type(options, "qoz:intrep_algo_str", pressio_option_charptr_type);//hided due to code
    // update
  ...
  struct pressio_options get_configuration_impl() const override {
    ...
    // set(options, "qoz:intrep_algo_str", keys(sz3_options().interp_algo));//hided due to code
    // update
    ...
    std::vector<std::string> invalidations{"qoz:abs_error_bound",
                                           ...
                                           " qoz: intrep_algo_str ",
                                           " qoz : algorithm_str ",
                                           " qoz: tuning_target_str "};
    std::vector<pressio_configurable const *> invalidation_children{};

    set(options, "predictors:error_dependent",
        get_accumulate_configuration("predictors:error_dependent", invalidation_children,
                                     invalidations));
    set(options, "predictors:error_agnostic",
        get_accumulate_configuration("predictors:error_agnostic", invalidation_children,
                                     invalidations));
    set(options, "predictors:runtime",
        get_accumulate_configuration("predictors:runtime", invalidation_children, invalidations));

    set(options, "pressio:highlevel",
        get_accumulate_configuration("pressio:highlevel", invalidation_children,
                                     std::vector<std::string>{"pressio:abs", "pressio:rel"}));
  ...
  struct pressio_options get_documentation_impl() const override {
    ...
    set(options, "pressio:description",
        R"(QoZ is the evolution of SZ3 a modular compression framework)");
    ...
    set(options, "qoz:qoz_level",
        "optimization level of QoZ compression. 0/1/2/3/4 are available. 0 is SZ3, 1 is QoZ1, 2 "
        "or 3 are recommended for QoZ2.");
  ...
  int major_version() const override { return 1; }
